...
#if defined(NDEBUG)
# error "Wekoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x104bc92aa0580eaf958555b02e750fdc04b7333252334aace1cd79739da6dde8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wekoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wekoin Signed Message:\n";
...
{
    // Wekoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Wekoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Wekoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Wekoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wekoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wekoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1420179359;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1050685;
        ...
        // Old Merkle Root: 97ddfbbae6be97fd6cdf3e7ca13232a3afff2353e29badfab7f73011edd4ced9
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Wekoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WekoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WekoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WekoinMiner(CWallet *pwallet)
{
    printf("WekoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wekoin-miner");
    ...
    try { loop {
        ...
        printf("Running WekoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WekoinMiner, pwallet));
}
